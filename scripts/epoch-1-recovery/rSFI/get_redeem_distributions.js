...
const fromBlock = program.fromblock ? program.fromblock : 11374859
const toBlock = program.toblock ? program.toblock : 11394311
...
async function writeRedeemEvents() {
    for (const addressesKey in addresses) {
        ...
        let logName = `${addressesKey.substring(0,8)}-${addresses[addressesKey]}-log.json`;
